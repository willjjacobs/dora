...
try:
    ...
    print("pylibfreenect2 not installed - cannot use Kinect")
...
class Kinect(object):
    ...
    def get_registered(self):
        ...
        self.registration.apply(color, depth, self.undistorted,
                                self.registered)
        self.listener.release(frame)
        return cv2.cvtColor(
            self.registered.asarray(np.uint8).copy(),
            cv2.COLOR_BGRA2BGR), new_depth
...
def calculate_heights(depth, camera_height, fov=[70.6, 60]):
    ...
    for i in range(0, size[0]):
        ...
        for j in range(0, size[1]):
            new_depth[i][j] = (camera_height - new_depth[i][j] * np.cos(alpha)
                               - new_depth[i][j] * np.cos(kth0))
    return new_depth
...
def overlay_image(image, dto, overlay_edges=True, isolate_sports_ball=False, threshold = .15):
    ...
    # Get indices and display *ONLY* sports balls
    # Index derived from mscoco_label_map.pbtext
    # Change the below flag to false if you want to view all detected
    # items and not just tennis balls.
